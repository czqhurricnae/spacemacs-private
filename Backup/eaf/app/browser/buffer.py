...
class AppBuffer(BrowserBuffer):
    def __init__(self, buffer_id, url, arguments):
        ...
        self.buildin_script_dir = os.path.join(os.path.dirname(__file__), "script")
        self.buffer_widget.loadProgress.connect(lambda: self.eval_script_file("merriam-webster.js"))
    ...
    def get_script_files(self):
        return [name for name in os.listdir(self.buildin_script_dir)
                if os.path.isfile(os.path.join(self.buildin_script_dir, name))]

    def read_script_content(self, script_file):
        ''' Read content of JavaScript(scripte) files.'''
        return open(os.path.join(self.buildin_script_dir, script_file), "r").read()

    def eval_script_file(self, script_file):
        ''' Run JavaScript from SCRIPT file.'''
        self.buffer_widget.eval_js(self.read_script_content(script_file))
