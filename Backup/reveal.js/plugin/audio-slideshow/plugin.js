...
//console.warn(videoElements[i]);
	...
	function setupAllAudioElements( container, h, v, slide ) {
      ...
      setupAudioElement( container, h + '.' + v, slide.getAttribute( 'data-audio-src' ), slide.getAttribute( 'data-track-src' ), '', null  );
			fragments = slide.querySelectorAll( 'code.fragment' );
			for ( i = 0; i < fragments.length; i++ ) {
          setupAudioElement( container, h + '.' + v + '.' + i, null, null, '', null  );
			}
			return;
		}
...
// console.log( h + '.' + v + ": " + text );
		  ...
		  setupAudioElement( container, h + '.' + v, slide.getAttribute( 'data-audio-src' ), slide.getAttribute( 'data-track-src' ), text, slide.querySelector( ':not(.fragment) > video[data-audio-controls]' ) );
		var i = 0;
		var  fragments;
      ...
      var track = null;
			...
			for( var f = 0, len = fragments.length; f < len; f++ ) {
				...
				if ( !track ) track = fragments[ f ].getAttribute( 'data-track-src' );
...
//console.log( h + '.' + v + '.' + i  + ": >" + text +"<")
			  setupAudioElement( container, h + '.' + v + '.' + i, audio, track, text, video  );
 			i++;
		}
	}
	...
	function setupFallbackAudio( audioElement, text, videoElement ) {
	  ...
	  function setupAudioElement( container, indices, audioFile, trackFile, text, videoElement ) {
		var audioElement = document.createElement( 'audio' );
    var trackElement = document.createElement('track');
    trackElement.src = trackFile;
