...
(defun extract-content-from-stackoverflow-to-org-file (src-code-type)
  (interactive
   (let ((src-code-type
    ...
    (setq org-image-url-template (concat "[[file:screenshotImg/" file-name "/\\1"))
    ...
    (setq stackoverflow-all-images-url-list (get--all-images-url html-file-name stackoverflow-image-url-pattern-list))
...
(defun get--all-images-url (file image-url-pattern-list)
...
(defun extract-content-from-official-accounts-to-org-file ()
  (interactive)
    ;; Read `url' string from minibuffer, while the string read is empty, this loop will not stop.
    (setq url "")
    (setq file-name "")
    ...
    (setq official-accounts-all-images-url-list (get--all-images-url html-file-name official-accounts-image-url-pattern-list))
